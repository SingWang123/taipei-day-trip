...
# 依據付款狀態, order_history 內新增付款紀錄
import random
import string

def order_history(status, name, id, data_id, date, time, price):
	try:
		# 連線到資料庫
		connection = pool2.get_connection()
		mycursor = connection.cursor(dictionary = True)
		...
		# 依據成功或失敗寫入不同的狀態
		if status == "success":
			sql = "INSERT INTO order_history (name, member_id, order_number, status, data_id, date, time, price) VALUES (%s, %s, %s, 'PAID', %s, %s, %s, %s)"
		else:
			sql = "INSERT INTO order_history (name, member_id, order_number, status, data_id, date, time, price) VALUES (%s, %s, %s, 'UNPAID', %s, %s, %s, %s)"
		
		val = (name, id, order_number, data_id, date, time, price)
		mycursor.execute(sql,val)
		connection.commit()
